...
public class ComputerPolicyDTO extends PolicyDTO{
    ...
    public ComputerPolicyDTO(ComputerPolicy computerPolicy) {
        ...
        this.windowsServices = computerPolicy.
                        getServices().
                        stream().
                        map(service -> new ConfigElementDTO<>(service)).
                        collect(Collectors.toList());
        this.computers = computerPolicy.getComputers().stream().map(computer -> new SimpleComputerDTO(computer)).collect(Collectors.toList());
    }
    ...
    public ComputerPolicy getComputerPolicy() {
        ComputerPolicy policy = new ComputerPolicy(this.getName(),
                this.getWindowsServices().
                        stream().
                        map(configElementDTO -> (WindowsService) configElementDTO.getConfigElement(new WindowsService())).
                        collect(Collectors.toList()));
